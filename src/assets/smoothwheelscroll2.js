...
.on('mousewheel wheel DOMMouseScroll', function(e){
  ...
  */
  ...
  //}
  //var top = Math.max(Math.min(pixelY * (spinY * 10), 500), 14) * delta.directionY;
  var top = Math.min(pixelY, PAGE_HEIGHT) * delta.directionY;

  top = Math.max($(window).scrollTop() + top, 0);
  

  //console.log(delta.spinY, delta.pixelY);
  //console.log(pixelY, spinY, JSON.stringify(delta));

  if (_ttwn){
    _ttwn.kill();
  }

  //var time = isInt(spinY) ? .5 : spinY;
